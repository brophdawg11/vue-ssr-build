...
const vsrVersion = require('vue-server-renderer/package.json').version;

const { config } = require('../js/isomorphic-utils');
const logger = require('../js/logger');

const USE_STREAM = true;
const serverInfo = `express/${expressVersion} vue-server-renderer/${vsrVersion}`;
...
const handleError = (err, res) => {
    if (err.url) {
        logger.debug('Redirecting from SSR error:', err.url);
...
function render(req, res) {
    ...
    if (!config.isProd) {
        logger.log(`Request took: ${Date.now() - s}ms`);
    }
}

module.exports = function initVueRenderer(app) {
    ...
    return config.isProd ? render : (req, res) => {
